...
@section('title', $item->name . ' - Scents N Smile')
...
<div class="min-h-screen bg-[#FAF8F5]">
    ...
    <!-- FILTER SIDEBAR -->
    <aside id="filterSidebar" class="fixed top-0 left-0 h-full w-[300px] bg-white z-[70] transform -translate-x-full transition-transform duration-300 ease-out overflow-y-auto">
        <div class="p-5">
            <!-- Close Button -->
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    <select id="sortSelect" class="w-full appearance-none bg-[#F4ECDD] rounded-full px-4 py-3 text-sm font-medium text-gray-900 cursor-pointer focus:outline-none">
                    ...
                    <svg class="w-4 h-4 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="border-b border-gray-100">
                ...
                <div id="genderFilter" class="pb-4 space-y-3">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="gender[]" value="{{ $gender->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'gender' && $item->id === $gender->id ? 'checked' : '' }}>
                        <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $gender->name }}</span>
                    </label>
                    @empty
                    <!-- Static fallback if no genders in database -->
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
            ...
            <div class="border-b border-gray-100">
                ...
                <div id="scentFilter" class="pb-4 space-y-3">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        ...
                        @else
                        <span class="w-4 h-4 rounded-full" style="background-color: {{ $tag->color ?? '#F27F6E' }}"></span>
                        @endif
                        <input type="checkbox" name="tag[]" value="{{ $tag->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'tag' && $item->id === $tag->id ? 'checked' : '' }}>
                        <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $tag->name }}</span>
                    </label>
                    @empty
            ...
            <div class="border-b border-gray-100">
                <button data-accordion-toggle="scentFamiliesFilter" class="w-full flex items-center justify-between py-4">
                ...
                <div id="scentFamiliesFilter" class="pb-4 space-y-3">
                    @forelse($scentFamilies as $scentFamily)
                    <label class="flex items-center gap-3 cursor-pointer group">
                        @if($scentFamily->icon)
                        <span class="text-base">{{ $scentFamily->icon }}</span>
                        @endif
                        <input type="checkbox" name="scent_family[]" value="{{ $scentFamily->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
                        <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $scentFamily->name }}</span>
                    </label>
                    @empty
            ...
            <div class="border-b border-gray-100">
                ...
                <div id="collectionsFilter" class="pb-4 space-y-3">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="collection[]" value="{{ $collection->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'collection' && $item->id === $collection->id ? 'checked' : '' }}>
                        <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $collection->name }}</span>
                    </label>
                    @empty
            ...
            <div class="border-b border-gray-100">
                <button data-accordion-toggle="highlightNotesFilter" class="w-full flex items-center justify-between py-4">
                ...
                <div id="highlightNotesFilter" class="pb-4 space-y-3">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        ...
                        <img src="{{ $note->imagekit_thumbnail_url ?? $note->imagekit_url }}" alt="{{ $note->name }}" class="w-6 h-6 rounded object-cover">
                        @else
                        <div class="w-6 h-6 rounded bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center">
                        ...
                        @endif
                        <input type="checkbox" name="highlight_note[]" value="{{ $note->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
                        <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $note->name }}</span>
                    </label>
                    @empty
            ...
            <div class="border-b border-gray-100">
                ...
                <div id="priceFilter" class="pb-4">
                    ...
                    <div class="price-slider-container mt-6 mb-2">
                        ...
                        <input type="range" id="priceRangeMin" min="0" max="10000" value="0" step="100" class="price-slider-thumb price-slider-thumb-left">
                        <input type="range" id="priceRangeMax" min="0" max="10000" value="10000" step="100" class="price-slider-thumb price-slider-thumb-right">
                    </div>
            ...
            <div class="mt-6">
                <button id="applyFilterBtn" class="w-full py-3 bg-[#F27F6E] text-white rounded-full font-medium hover:bg-[#e06b5a] transition-colors">
                    Apply Filters
                </button>
            </div>
        </div>
        ...
        <div>
            <div class="flex gap-4 overflow-x-auto pb-2 hide-scrollbar pl-4">
                ...
                <a href="{{ route('collections.show', 'all') }}" 
                   class="flex-shrink-0 w-[320px] h-24 rounded-[28px] flex items-center justify-between px-8 relative overflow-hidden transition-transform hover:scale-[1.02] {{ $slug === 'all' ? 'bg-[#F27F6E]' : 'bg-[#F4ECDD]' }}">
                    <div class="flex flex-col justify-center z-10">
                        <span class="{{ $slug === 'all' ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">All Perfumes</span>
                        <span class="{{ $slug === 'all' ? 'text-white/90' : 'text-gray-600' }} text-sm font-medium uppercase tracking-wider">Farewell Sale</span>
                    </div>
                    <div class="absolute right-4 top-1/2 -translate-y-1/2">
                        <img src="https://images.unsplash.com/photo-1541643600914-78b084683601?w=100&h=120&fit=crop" alt="Perfume" class="h-20 w-auto object-contain drop-shadow-lg">
                    </div>
                </a>

                <!-- Card 2: BESTSELLERS -->
                <a href="{{ route('collections.show', 'bestsellers') }}" 
                   class="flex-shrink-0 w-[280px] h-24 rounded-[28px] flex items-center justify-between px-8 relative overflow-hidden transition-transform hover:scale-[1.02] {{ $slug === 'bestsellers' ? 'bg-[#F27F6E]' : 'bg-[#F4ECDD]' }}">
                    <div class="flex flex-col justify-center z-10">
                        <span class="{{ $slug === 'bestsellers' ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">Bestsellers</span>
                    </div>
                    <div class="absolute right-2 top-1/2 -translate-y-1/2">
                        <img src="https://images.unsplash.com/photo-1594035910387-fea47794261f?w=100&h=120&fit=crop" alt="Bestsellers" class="h-20 w-auto object-contain drop-shadow-lg">
                    </div>
                </a>

                <!-- Card 3: NEW ARRIVALS -->
                <a href="{{ route('collections.show', 'new-arrivals') }}" 
                   class="flex-shrink-0 w-[280px] h-24 rounded-[28px] flex items-center justify-between px-8 relative overflow-hidden transition-transform hover:scale-[1.02] {{ $slug === 'new-arrivals' ? 'bg-[#F27F6E]' : 'bg-[#F4ECDD]' }}">
                    <div class="flex flex-col justify-center z-10">
                        <span class="{{ $slug === 'new-arrivals' ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">New Arrivals</span>
                    </div>
                    <div class="absolute right-4 top-1/2 -translate-y-1/2">
                        <img src="https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=80&h=120&fit=crop" alt="New Arrivals" class="h-20 w-auto object-contain drop-shadow-lg">
                    </div>
                </a>
                ...
                <a href="{{ route('collections.show', $gender->slug) }}" 
                   class="flex-shrink-0 w-[240px] h-24 rounded-[28px] flex items-center justify-between px-8 relative overflow-hidden transition-transform hover:scale-[1.02] {{ $type === 'gender' && $item->id === $gender->id ? 'bg-[#F27F6E]' : 'bg-[#F4ECDD]' }}">
                    <div class="flex flex-col justify-center z-10">
                        <span class="{{ $type === 'gender' && $item->id === $gender->id ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">{{ $gender->name }}</span>
                    </div>
                    @if($gender->imagekit_thumbnail_url || $gender->imagekit_url)
                    <div class="absolute right-4 top-1/2 -translate-y-1/2">
                        <img src="{{ $gender->imagekit_thumbnail_url ?? $gender->imagekit_url }}" alt="{{ $gender->name }}" class="h-16 w-auto object-contain drop-shadow-lg">
                    </div>
                    @endif
                </a>
                @endforeach
    ...
    <section class="pb-6">
        <div class="px-4">
            <div class="flex flex-wrap items-center gap-3 lg:gap-4">
                ...
                <!-- Sort & Filter Button -->
                <button id="sortFilterBtn" class="flex items-center gap-2 px-6 py-3 border border-gray-900 rounded-full text-sm font-medium text-gray-900 hover:bg-gray-900 hover:text-white transition-colors">
                ...
                <div class="flex-1 min-w-[280px] relative">
                    <input type="text" id="searchInput" placeholder="Search scents, ingredients" class="w-full pl-5 pr-14 py-3 border border-gray-900 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#F27F6E] focus:border-transparent bg-white">
                    <button id="searchBtn" class="absolute right-1.5 top-1/2 -translate-y-1/2 w-10 h-10 bg-[#F27F6E] rounded-full flex items-center justify-center hover:bg-[#e06b5a] transition-colors">
                ...
                <!-- Gender Filter Pill -->
                <div class="relative">
                    <button id="genderPillBtn" class="flex items-center gap-2 px-5 py-3 bg-[#F4ECDD] rounded-full text-sm font-medium text-gray-900 hover:bg-[#ebe3d4] transition-colors">
                    ...
                    <div id="genderPillDropdown" class="hidden absolute top-full mt-2 left-0 bg-white rounded-2xl shadow-xl border border-gray-200 p-4 min-w-[200px] z-50">
                        @foreach($genders as $gender)
                        <label class="flex items-center gap-3 cursor-pointer group py-2 hover:bg-gray-50 rounded-lg px-2">
                            <input type="checkbox" name="gender_pill[]" value="{{ $gender->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'gender' && $item->id === $gender->id ? 'checked' : '' }}>
                            <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $gender->name }}</span>
                        </label>
                        @endforeach
                    </div>
                ...
                <div class="relative">
                    <button id="scentFamilyPillBtn" class="flex items-center gap-2 px-5 py-3 bg-[#F4ECDD] rounded-full text-sm font-medium text-gray-900 hover:bg-[#ebe3d4] transition-colors">
                    ...
                    <div id="scentFamilyPillDropdown" class="hidden absolute top-full mt-2 left-0 bg-white rounded-2xl shadow-xl border border-gray-200 p-4 min-w-[200px] z-50">
                        @forelse($scentFamilies as $scentFamily)
                        <label class="flex items-center gap-3 cursor-pointer group py-2 hover:bg-gray-50 rounded-lg px-2">
                            @if($scentFamily->icon)
                            <span class="text-base">{{ $scentFamily->icon }}</span>
                            @endif
                            <input type="checkbox" name="scent_family_pill[]" value="{{ $scentFamily->id }}" class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
                            <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $scentFamily->name }}</span>
                        </label>
                        @empty
        ...
        <div class="px-4">
            ...
            <div class="flex items-center justify-between mb-6">
                <p class="text-gray-600 text-sm">
                    ...
                    <span id="productCount" class="font-semibold text-gray-900 ml-2">{{ $products->total() }} Products</span>
                </p>
            </div>
            ...
            <div id="productsGrid">
                <div id="productsContainer">
                    <div class="text-center py-20">
                        ...
                        <a href="{{ route('collections.show', 'all') }}" class="inline-flex items-center px-8 py-3 bg-[#F27F6E] text-white rounded-full font-medium hover:bg-[#e06b5a] transition-colors">
                            Browse All Perfumes
                        </a>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyFilters() {
        ...
        // AJAX request
        fetch('{{ route("collections.filter") }}', {
            method: 'POST',
            headers: {
            ...
            loadMoreBtn.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (filters) {
                        ...
                        response = await fetch('{{ route("collections.filter") }}', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                                'Accept': 'application/json'
