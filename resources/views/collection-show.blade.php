                        ...
                        @forelse($genders as $gender)
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" name="gender[]" value="{{ $gender->id }}"
                                    class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'gender' && $item->id === $gender->id ? 'checked' : '' }}>
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                @else
                                    <span class="w-4 h-4 rounded-full"
                                        style="background-color: {{ $tag->color ?? '#F27F6E' }}"></span>
                                @endif
                                <input type="checkbox" name="tag[]" value="{{ $tag->id }}"
                                    class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'tag' && $item->id === $tag->id ? 'checked' : '' }}>
                    ...
                    <div id="scentFamiliesFilter" class="pb-4 space-y-3">
                        @forelse($scentFamilies as $scentFamily)
                            <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                @endif
                                <input type="checkbox" name="scent_family[]" value="{{ $scentFamily->id }}"
                                    class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
                        ...
                        @forelse($collections as $collection)
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" name="collection[]" value="{{ $collection->id }}"
                                    class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'collection' && $item->id === $collection->id ? 'checked' : '' }}>
                        ...
                        @forelse($highlightNotes as $note)
                            <label class="flex items-center gap-3 cursor-pointer group">
                                @if($note->imagekit_thumbnail_url || $note->imagekit_url)
                                    <img src="{{ $note->imagekit_thumbnail_url ?? $note->imagekit_url }}" alt="{{ $note->name }}"
                                ...
                                @endif
                                <input type="checkbox" name="highlight_note[]" value="{{ $note->id }}"
                                    class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
                ...
                <div class="flex gap-4 overflow-x-auto pb-2 hide-scrollbar pl-4">
                    ...
                    <a href="{{ route('collections.show', 'all') }}"
                        ...
                        <div class="flex flex-col justify-center z-10">
                            <span
                                class="{{ $slug === 'all' ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">All
                                Perfumes</span>
                            <span
                                class="{{ $slug === 'all' ? 'text-white/90' : 'text-gray-600' }} text-sm font-medium uppercase tracking-wider">Farewell
                                Sale</span>
                        </div>
                    ...
                    <a href="{{ route('collections.show', 'bestsellers') }}"
                        ...
                        <div class="flex flex-col justify-center z-10">
                            <span
                                class="{{ $slug === 'bestsellers' ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">Bestsellers</span>
                        </div>
                    ...
                    <a href="{{ route('collections.show', 'new-arrivals') }}"
                        ...
                        <div class="flex flex-col justify-center z-10">
                            <span
                                class="{{ $slug === 'new-arrivals' ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">New
                                Arrivals</span>
                        </div>
                        <div class="absolute right-4 top-1/2 -translate-y-1/2">
                    ...
                    @foreach($genders as $gender)
                        <a href="{{ route('collections.show', $gender->slug) }}"
                            ...
                            <div class="flex flex-col justify-center z-10">
                                <span
                                    class="{{ $type === 'gender' && $item->id === $gender->id ? 'text-white' : 'text-gray-900' }} text-lg font-bold uppercase tracking-wide">{{ $gender->name }}</span>
                            </div>
                            @if($gender->imagekit_thumbnail_url || $gender->imagekit_url)
                                <div class="absolute right-4 top-1/2 -translate-y-1/2">
                                    <img src="{{ $gender->imagekit_thumbnail_url ?? $gender->imagekit_url }}"
                                        alt="{{ $gender->name }}" class="h-16 w-auto object-contain drop-shadow-lg">
                                </div>
                            @endif
                        </a>
                    @endforeach
                        ...
                        <div id="genderPillDropdown"
                            ...
                            @foreach($genders as $gender)
                                <label
                                    ...
                                    <input type="checkbox" name="gender_pill[]" value="{{ $gender->id }}"
                                        class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]" {{ $type === 'gender' && $item->id === $gender->id ? 'checked' : '' }}>
                                    <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ $gender->name }}</span>
                                </label>
                            @endforeach
                            ...
                            @forelse($scentFamilies as $scentFamily)
                                <label
                                    ...
                                    @if($scentFamily->icon)
                                        <span class="text-base">{{ $scentFamily->icon }}</span>
                                    @endif
                                    <input type="checkbox" name="scent_family_pill[]" value="{{ $scentFamily->id }}"
                                        class="w-4 h-4 rounded border-gray-300 text-[#F27F6E] focus:ring-[#F27F6E]">
                                    <span
                                        class="text-sm text-gray-700 group-hover:text-gray-900">{{ $scentFamily->name }}</span>
                                </label>
                            @empty
            ...
            <div class="px-4">
                ...
                <div class="flex items-center justify-between mb-6">
                    <p class="text-gray-600 text-sm">
                        ...
                        <span id="productCount" class="font-semibold text-gray-900 ml-2">{{ $products->total() }}
                            Products</span>
                    </p>
                </div>
                ...
                @if($products->hasMorePages())
                    <div id="loadMoreContainer" class="mt-12 flex justify-center">
                        <button id="loadMoreBtn" data-next-page="{{ $products->currentPage() + 1 }}"
