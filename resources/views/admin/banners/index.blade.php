                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-200">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-gray-900 mt-2">{{ $totalBanners }}</h3>
                        </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-200">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-green-600 mt-2">{{ $activeBanners }}</h3>
                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-200">
                <div class="p-6">
                    <div id="banners-list" class="space-y-4">
                        ...
                        <div data-banner-id="{{ $banner->id }}" class="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border-2 border-gray-200 hover:border-blue-300 transition-all cursor-move">
                            ...
                            @if($banner->image)
                                <img src="{{ $banner->image }}" alt="{{ $banner->title }}" class="w-32 h-20 object-cover rounded-lg">
                            @else
                            ...
                            <div class="flex-1">
                                ...
                                @if($banner->link)
                                <a href="{{ $banner->link }}" target="_blank" class="text-xs text-blue-600 hover:underline">{{ Str::limit($banner->link, 40) }}</a>
                                @endif
                            </div>
                            <div class="flex items-center gap-3">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" {{ $banner->is_active ? 'checked' : '' }} onchange="toggleBanner({{ $banner->id }}, this.checked)" class="sr-only peer">
                                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                                </label>
                                <button onclick="editBanner({{ $banner->id }})" class="p-2 bg-yellow-100 hover:bg-yellow-200 rounded-lg text-yellow-700">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button onclick="deleteBanner({{ $banner->id }})" class="p-2 bg-red-100 hover:bg-red-200 rounded-lg text-red-700">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function uploadToImageKit(file) {
    ...
    fetch('{{ route("admin.banners.uploadImage") }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
document.getElementById('bannerForm').addEventListener('submit', function(e) {
    ...
    const url = bannerId ? `/admin/banners/${bannerId}` : '{{ route("admin.banners.store") }}';
...
function fillDemoData() {
    showConfirmDialog(
        ...
        () => {
            fetch('{{ route("admin.banners.fillDemo") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
