...
@section('step_description', 'Add size variants for your product')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
                        ...
                        <label class="cursor-pointer group">
                            <input type="checkbox" name="selected_sizes[]" value="{{ $size->id }}" class="hidden size-checkbox" data-size-name="{{ $size->name }}">
                            <div class="relative">
                                <div class="h-16 rounded-lg border-2 border-gray-300 transition-all duration-200 flex flex-col items-center justify-center bg-white hover:border-indigo-400 hover:shadow-md size-box">
                                    <span class="font-bold text-lg text-gray-700">{{ $size->abbreviation ?? substr($size->name, 0, 3) }}</span>
                                    <span class="text-xs text-gray-500 mt-0.5">{{ $size->name }}</span>
                                </div>
