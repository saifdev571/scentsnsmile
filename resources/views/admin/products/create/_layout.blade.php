        ...
        <div class="w-full px-4 sm:px-6 py-5">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3 sm:space-x-5">
                    ...
                    <div>
                        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">@yield('step_title', 'Create Product')</h1>
                        <p class="text-sm sm:text-base text-gray-600 font-medium">@yield('step_description', 'Build your product catalog')</p>
                    </div>
                </div>
                        ...
                        @foreach($steps as $index => $step)
                            ...
                            <div class="flex flex-col items-center space-y-1 sm:space-y-2 cursor-pointer group step-indicator {{ $step['number'] == $current ? 'active' : '' }} {{ $step['number'] < $current ? 'completed' : '' }}" 
                                ...
                                <div class="text-center">
                                    <div class="text-xs sm:text-sm font-semibold whitespace-nowrap {{ $step['number'] == $current ? 'text-gray-900' : ($step['number'] < $current ? 'text-green-700' : 'text-gray-600') }}">{{ $step['title'] }}</div>
                                    <div class="text-xs whitespace-nowrap {{ $step['number'] == $current ? 'text-gray-500' : ($step['number'] < $current ? 'text-green-500' : 'text-gray-400') }} hidden sm:block">{{ $step['description'] }}</div>
                                </div>
                            </div>

                            @if($index < count($steps) - 1)
                                <div class="w-6 sm:w-10 md:w-12 lg:w-16 xl:w-20 h-0.5 rounded-full progress-line {{ $step['number'] < $current ? 'active bg-gradient-to-r from-green-500 to-green-600' : 'bg-gray-300' }}"></div>
                            @endif
                        @endforeach
                    </div>
                </div>
