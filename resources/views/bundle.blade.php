...
@section('title', 'Bundle - Scents N Smile')
...
@section('content')
                            ...
                            <div class="overflow-hidden">
                                <div id="carouselTrack"
                                    ...
                                    <div
                                        ...
                                        <button
                                            class="add-item-btn text-red-400 text-2xl sm:text-3xl mb-0.5 sm:mb-1">+</button>
                                        <span class="slot-number text-lg sm:text-xl md:text-2xl font-light text-gray-400">{{ $i }}</span>
                                    </div>
                                    @endfor
                                </div>
                            </div>
                    ...
                    <section class="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                            @foreach($products as $product)
                                @include('partials.product-card', ['product' => $product, 'buttonText' => 'ADD TO BUNDLE', 'buttonClass' => 'add-to-bundle-btn'])
                            @endforeach
                        </div>
                    </section>
                </div>
    ...
    <script>
        ...
        @endphp
        let bundleData = @json($bundleData);

        // Load existing bundle from database
        @if(isset($existingBundle) && $existingBundle)
            bundleItems = @json($existingBundle->items);
