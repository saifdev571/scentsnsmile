...
@section('title', 'Checkout - Scents N Smile')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div class="border-2 border-gray-200 rounded-xl p-3 sm:p-4 cursor-pointer hover:border-[#e8a598] transition-colors address-card {{ $address->is_default ? 'border-[#e8a598] bg-[#e8a598]/5' : '' }}" 
                     data-address='@json($address)'>
                    <div class="flex items-start gap-3">
                        <input type="radio" name="selected_address" value="{{ $address->id }}" 
                               class="w-5 h-5 text-[#e8a598] mt-1 flex-shrink-0" {{ $address->is_default ? 'checked' : '' }}>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap mb-1">
                                <span class="font-bold text-gray-900">{{ $address->name }}</span>
                                <span class="text-xs px-2 py-0.5 rounded {{ $address->type === 'home' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700' }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="first_name" id="first_name" required
                                    ...
                                    value="{{ isset($defaultAddress) && $defaultAddress->name ? explode(' ', $defaultAddress->name)[0] : ($user->name ? explode(' ', $user->name)[0] : '') }}">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Last Name *</label>
                                <input type="text" name="last_name" id="last_name" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e8a598] focus:border-[#e8a598] outline-none transition-colors"
                                    value="{{ isset($defaultAddress) && $defaultAddress->name && count(explode(' ', $defaultAddress->name)) > 1 ? explode(' ', $defaultAddress->name, 2)[1] : ($user->name && count(explode(' ', $user->name)) > 1 ? explode(' ', $user->name, 2)[1] : '') }}">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex">
                                ...
                                <input type="tel" name="phone" id="phone" required maxlength="10"
                                    ...
                                    value="{{ isset($defaultAddress) && $defaultAddress->phone ? preg_replace('/[^0-9]/', '', str_replace('+91', '', $defaultAddress->phone)) : ($user->phone ?? '') }}" placeholder="10-digit mobile number">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="address" id="address" required
                                ...
                                value="{{ isset($defaultAddress) ? $defaultAddress->address_line_1 : '' }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="address_line_2" id="address_line_2"
                                ...
                                value="{{ isset($defaultAddress) ? $defaultAddress->address_line_2 : '' }}">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="zipcode" id="zipcode" required maxlength="6"
                                    ...
                                    value="{{ isset($defaultAddress) ? $defaultAddress->pincode : '' }}">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">City *</label>
                                <input type="text" name="city" id="city" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e8a598] focus:border-[#e8a598] outline-none transition-colors"
                                    value="{{ isset($defaultAddress) ? $defaultAddress->city : '' }}">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="state" id="state" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e8a598] focus:border-[#e8a598] outline-none transition-colors"
                                value="{{ isset($defaultAddress) ? $defaultAddress->state : '' }}">
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl shadow-sm sticky top-24">
                    ...
                    <div class="bg-gradient-to-r from-[#e8a598] to-[#F27F6E] px-4 sm:px-6 py-4 rounded-t-2xl">
                        ...
                        <p class="text-white/80 text-sm">{{ $itemCount }} item(s)</p>
                    </div>
                    ...
                    <div class="p-4 sm:p-6 border-b max-h-64 overflow-y-auto">
                        ...
                        <div class="flex gap-3 mb-4 last:mb-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="font-semibold text-sm text-gray-900 line-clamp-2">{{ $item->product->name }}</h4>
                                <p class="text-gray-500 text-xs mt-1">Qty: {{ $item->quantity }}</p>
                                <p class="font-bold text-sm mt-1">₹{{ number_format($item->price * $item->quantity, 0) }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <div class="p-4 sm:p-6 space-y-3 text-sm">
                        ...
                        @if($discount > 0)
                        <div class="flex justify-between" style="color: {{ $activePromotion->badge_color ?? '#e8a598' }}">
                            <span class="font-medium">{{ $activePromotion->cart_label ?? 'Discount' }} ({{ $activePromotion ? $activePromotion->discount_text : '40% OFF' }})</span>
                        ...
                        <div class="border-t pt-3 mt-3">
                            <div class="flex justify-between text-lg">
                                ...
                                <span class="font-bold text-[#e8a598]">₹{{ number_format($total, 0) }}</span>
                            </div>
...
<script>
(function() {
    var razorpayEnabled = {{ $razorpayEnabled ? 'true' : 'false' }};
