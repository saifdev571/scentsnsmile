...
@section('content')
    <div class="font-sans antialiased text-gray-900 bg-white min-h-screen">
        ...
        {{-- Main Container --}}
        <div class="w-full flex flex-col gap-4" x-data="{ activeFamily: '{{ $activeFamily ? $activeFamily->slug : ($allFamilies->first()->slug ?? '') }}' }">

            @foreach($allFamilies as $family)
                <div class="w-full transition-all duration-500 ease-in-out">
                    
                    {{-- COLLAPSED STATE: Card View --}}
                    <div x-show="activeFamily !== '{{ $family->slug }}'"
                         x-transition:enter="transition ease-out duration-300"
                         x-transition:enter-start="opacity-0 max-h-0"
                         x-transition:enter-end="opacity-100 max-h-96"
                         {{-- Note: Using simple show/hide mostly, but transitions help smoothness --}}
                         @click="activeFamily = '{{ $family->slug }}'"
                         class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 cursor-pointer">
                        
                        <div class="group block border-b border-gray-100 hover:bg-gray-50 transition-all duration-300 relative overflow-hidden rounded-3xl shadow-sm hover:shadow-md bg-white">
                    ...
                    <div x-show="activeFamily === '{{ $family->slug }}'"
                         ...
                         style="background-color: {{ $family->color_code ?? '#dcfce7' }};">
                        ...
                        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 relative z-10">
                            <div class="flex flex-col md:flex-row items-center gap-8 md:gap-12">
                                ...
                                <div class="w-full md:w-1/3 flex flex-col items-center md:items-start text-center md:text-left space-y-6 z-10">
                                    <div class="bg-white/90 backdrop-blur-md rounded-3xl p-8 shadow-xl border border-white/50 relative overflow-hidden">
                                        {{-- Decorative Elements --}}
                                        ...
                                        <div class="text-gray-700 text-lg leading-relaxed relative z-10 prose prose-sm max-w-none">
                                            {!! $family->description ?? 'Discover our collection of fresh, vibrant scents perfect for everyday wear.' !!}
                                        </div>
                                    </div>
                                </div>

                                {{-- Right Content: Product Carousel / Grid --}}
                                <div class="w-full md:w-2/3 relative z-10 group" id="products-grid-{{ $family->id }}">
                                    @if($family->products->isNotEmpty())
                                        ...
                                        <div id="scent-products-slider-{{ $family->id }}"
                                            class="flex overflow-x-auto gap-4 py-8 px-4 -mx-4 snap-x snap-proximity hide-scroll-bar scroll-smooth max-w-[784px] mx-auto">
