...
@if(isset($headerPromotion) && $headerPromotion && !request()->routeIs('home'))
    <div class="fixed top-0 left-0 right-0 z-[51] px-2 pt-1.5">
        <div class="text-white rounded-full mx-auto max-w-7xl"
            style="background-color: {{ $headerPromotion->badge_color ?? '#000000' }}">
            <div class="px-4 py-1.5">
                <div class="flex items-center justify-between text-xs">
                    <!-- Left - Sale Name -->
                    <div class="flex items-center gap-2">
                        <span
                            class="font-black uppercase tracking-wider text-sm">{{ $headerPromotion->banner_title ?? $headerPromotion->name }}</span>
                    </div>
                    ...
                    <div class="hidden md:flex items-center gap-4 lg:gap-6">
                        @if($headerPromotion->discount_value > 0)
                            <div class="flex items-center gap-1.5">
                                <span
                                    ...
                                    style="color: {{ $headerPromotion->badge_color ?? '#000000' }}">{{ $headerPromotion->min_items }}</span>
                        ...
                        @if($headerPromotion->free_shipping)
                            ...
                            <div class="flex items-center gap-1.5">
                                <span
                                    class="bg-white rounded-full w-5 h-5 flex items-center justify-center font-bold text-[10px]"
                                    style="color: {{ $headerPromotion->badge_color ?? '#000000' }}">{{ $headerPromotion->free_shipping_min_items ?? 3 }}</span>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="bg-white rounded-xl overflow-hidden p-1">
                                        @foreach($categories as $category)
                                            <a href="{{ route('collections.show', $category->slug) }}"
                                                class="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg transition-colors">
                                                @if($category->imagekit_url)
                                                    <img src="{{ $category->imagekit_thumbnail_url ?? $category->imagekit_url }}"
                                                    ...
                                                    <div
                                                        ...
                                                        <span
                                                            class="text-xs font-bold text-blue-600">{{ substr($category->name, 0, 1) }}</span>
                                                    </div>
                                                @else
                                                    <div
                                        ...
                                        <a href="{{ route('collections.show', $gender->slug) }}"
                                            ...
                                            @if($gender->imagekit_url)
                                                <img src="{{ $gender->imagekit_thumbnail_url ?? $gender->imagekit_url }}"
                                                    alt="{{ $gender->name }}" class="w-10 h-10 rounded-lg object-cover"
                                                    onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                                <div
                                                    class="w-10 h-10 rounded-lg bg-gray-200 items-center justify-center hidden">
                                                    <span
                                                        class="text-xs font-bold text-gray-500">{{ substr($gender->name, 0, 1) }}</span>
                                                </div>
                                            @else
                                        ...
                                        <a href="{{ route('collections.show', $tag->slug) }}"
                                            ...
                                            @if($tag->imagekit_url)
                                                <img src="{{ $tag->imagekit_thumbnail_url ?? $tag->imagekit_url }}"
                                                    alt="{{ $tag->name }}" class="w-10 h-10 rounded-lg object-cover"
                                                    onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
...
<div id="cartSidebar" class="fixed inset-0 z-[70] hidden">
    ...
    <div id="cartPanel"
        ...
        <div class="flex items-center justify-between p-2.5 border-b">
            ...
            <img src="{{ asset('images/logo-transparent.png') }}"
                alt="{{ $settings['business_name'] ?? 'Scents N Smile' }}" class="h-8 w-auto">
            <div class="w-6"></div>
        </div>
        ...
        @if(isset($activePromotion) && $activePromotion)
            <div class="text-white p-2.5 m-2.5 rounded-lg"
                style="background-color: {{ $activePromotion->badge_color ?? '#000000' }}" id="cartPromoBanner"
                data-promo-id="{{ $activePromotion->id }}" data-min-items="{{ $activePromotion->min_items }}"
                ...
                <div class="flex items-center gap-1.5 mb-2">
                    <h3 class="text-sm font-black uppercase tracking-wide">
                        {{ $activePromotion->banner_title ?? $activePromotion->name }}
                    </h3>
                ...
                @if($activePromotion->discount_value > 0)
                    ...
                    <div class="mb-2.5">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-[11px]">{{ $activePromotion->min_items }}+ items
                                {{ $activePromotion->discount_text }}</span>
                        </div>
                ...
                @if($activePromotion->free_shipping)
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-[11px]">{{ $activePromotion->free_shipping_min_items ?? 3 }}+ items Free
                                shipping</span>
        ...
        @else
            ...
            <div class="bg-gray-800 text-white p-2.5 m-2.5 rounded-lg" id="cartPromoBanner" data-promo-id="0"
                ...
                data-free-shipping-min="3" data-free-shipping-threshold="{{ $settings['free_shipping_threshold'] ?? 999 }}"
                data-shipping-charge="{{ $settings['shipping_charge'] ?? 99 }}" data-cart-label="Discount">
                <div class="text-center py-2">
                    @if(($settings['free_shipping_threshold'] ?? 999) > 0)
                        <p class="text-sm">Free shipping on orders above
                            ₹{{ number_format($settings['free_shipping_threshold'] ?? 999, 0) }}</p>
                    @else
                ...
                <div class="bg-[#f5e6d3] rounded-xl px-4 py-3 flex items-center gap-3 mb-3">
                    <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center">
                        <span class="text-lg font-bold"
                            style="color: {{ $activePromotion->badge_color ?? '#ef4444' }}">1</span>
                    </div>
            ...
            <div class="mb-3">
                <div class="border-2 border-red-400 rounded-xl overflow-hidden">
                    ...
                    <div class="bg-[#f5e6d3] px-4 py-3 flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center">
                            <span class="text-lg font-bold"
                                style="color: {{ $activePromotion->badge_color ?? '#ef4444' }}">2</span>
                        </div>
                    ...
                    <div class="bg-white px-4 py-4 space-y-3">
                        ...
                        <div class="flex justify-between text-base"
                            style="color: {{ $activePromotion->badge_color ?? '#ef4444' }}">
                            <span class="font-semibold"
                                id="cartDiscountLabel">{{ $activePromotion->cart_label ?? 'Sale discount' }}:</span>
                        ...
                        @foreach($categories as $category)
                            <a href="{{ route('collections.show', $category->slug) }}"
                                ...
                                @if($category->imagekit_url)
                                    ...
                                    <div
                                        class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-100 to-indigo-100 items-center justify-center hidden">
                                        <span class="text-xs font-bold text-blue-600">{{ substr($category->name, 0, 1) }}</span>
                                    </div>
                                @else
                                    <div
                                        class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center">
                ...
                <div class="bg-white rounded-xl overflow-hidden p-1">
                    @forelse($featuredTags ?? [] as $tag)
                        <a href="{{ route('collections.show', $tag->slug) }}"
                            class="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg transition-colors">
                            @if($tag->imagekit_url)
                                <img src="{{ $tag->imagekit_thumbnail_url ?? $tag->imagekit_url }}" alt="{{ $tag->name }}"
                                    class="w-10 h-10 rounded-lg object-cover"
                                    onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div class="w-10 h-10 rounded-lg items-center justify-center hidden"
                                    style="background-color: {{ $tag->color ?? '#3B82F6' }}20; border: 2px solid {{ $tag->color ?? '#3B82F6' }};">
