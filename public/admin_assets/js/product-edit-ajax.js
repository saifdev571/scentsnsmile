class ProductEditAjax {
    constructor() {
        this.csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
        this.currentStep = this.getCurrentStepNumber();
        this.totalSteps = 7;
    ...
    async submitStep(actionUrl, formData) {
        try {
            ...
            if (data.success) {
                this.showSuccess(data.message || 'Step saved successfully!');
                ...
                this.showErrors(data.errors || { general: [data.message || 'An error occurred'] });
            }
    ...
    navigateToNextStep() {
        ...
        if (currentStep < this.totalSteps) {
            const nextStepUrl = this.getEditStepUrl(currentStep + 1);
            this.navigateToStep(nextStepUrl);
        } else {
    ...
    goToPreviousStep() {
        const currentStep = this.getCurrentStepNumber();
        if (currentStep > 1) {
            const prevStepUrl = this.getEditStepUrl(currentStep - 1);
    ...
    getEditStepUrl(stepNumber) {
        const baseUrl = window.location.origin;
        return `${baseUrl}/admin/products/${this.productId}/edit/step${stepNumber}`;
    }
    ...
    showLoading(message = 'Saving...') {
        ...
        if (nextBtn) {
            ...
            nextBtn.setAttribute('data-original-text', originalText);
    ...
    hideLoading() {
        ...
        if (nextBtn) {
            ...
            const originalText = nextBtn.getAttribute('data-original-text');
            if (originalText) {
                nextBtn.innerHTML = originalText;
            } else {
                const currentStep = this.getCurrentStepNumber();
                if (currentStep === this.totalSteps) {
                    nextBtn.innerHTML = '✅ Update Product';
                } else {
                    nextBtn.innerHTML = `Continue to Step ${currentStep + 1} →`;
                }
            }
        }
...
// Initialize only once to prevent conflicts
if (typeof window.productEditAjax === 'undefined') {
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            window.productEditAjax = new ProductEditAjax();
        });
    } else {
        window.productEditAjax = new ProductEditAjax();
    }
