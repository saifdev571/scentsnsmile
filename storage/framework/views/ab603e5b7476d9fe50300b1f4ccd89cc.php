    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            ...
            <select name="status" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black focus:border-transparent">
                ...
                <option value="inactive" <?php echo e(request('status') === 'inactive' ? 'selected' : ''); ?>>Unsubscribed</option>
            </select>
            <button type="submit" class="px-6 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition">
                Filter
            </button>
            <?php if(request()->hasAny(['search', 'status'])): ?>
            <a href="<?php echo e(route('admin.newsletter.index')); ?>" class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition text-center">
                Clear
            </a>
            <?php endif; ?>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition" id="row-<?php echo e($subscriber->id); ?>">
                        <td class="px-6 py-4">
                            <input type="checkbox" class="subscriber-checkbox rounded border-gray-300" value="<?php echo e($subscriber->id); ?>">
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm font-medium text-gray-900"><?php echo e($subscriber->email); ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <span id="status-<?php echo e($subscriber->id); ?>" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo e($subscriber->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?>">
                                <?php echo e($subscriber->is_active ? 'Active' : 'Unsubscribed'); ?>

                            </span>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-500">
                            <?php echo e($subscriber->subscribed_at ? $subscriber->subscribed_at->format('d M Y, h:i A') : '-'); ?>

                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="toggleSubscriber(<?php echo e($subscriber->id); ?>)" 
                                        class="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition"
                                        title="<?php echo e($subscriber->is_active ? 'Deactivate' : 'Activate'); ?>">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/>
                                    </svg>
                                </button>
                                <button onclick="deleteSubscriber(<?php echo e($subscriber->id); ?>)" 
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/htdocs/scentnsmile/resources/views/admin/newsletter/index.blade.php ENDPATH**/ ?>